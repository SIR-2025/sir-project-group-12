...
Usage:
    uv run snowwhite_interactive.py
...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "../request_dialogflow"))
)
...
from sic_framework.devices.common_naoqi.naoqi_autonomous import (
    NaoBasicAwarenessRequest, NaoBlinkingRequest, NaoListeningMovementRequest,
    NaoRestRequest, NaoSetAutonomousLifeRequest, NaoWakeUpRequest)
from sic_framework.devices.common_naoqi.naoqi_motion import \
    NaoqiAnimationRequest
from sic_framework.devices.common_naoqi.naoqi_text_to_speech import \
    NaoqiTextToSpeechRequest
from sic_framework.services.google_stt.google_stt import (
    GetStatementRequest, GoogleSpeechToText, GoogleSpeechToTextConf)
...
try:
    ...
    print(
        "Error: Could not import dialogflow_cx_direct_request. Make sure the request_dialogflow directory is in sys.path."
    )
    sys.exit(1)
...
class SnowWhiteInteractive(SICApplication):
    ...
    def __init__(self):
        ...
        self.dialogflow_keyfile_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../conf/google/google-key.json")
        )
    ...
    def perform_script(self, script_lines, name="Script"):
        ...
        for i, line in enumerate(script_lines):
            ...
            self.logger.info(
                f"[{name} {i+1}] Saying: '{text[:20]}...' | Gesture: {gesture}"
            )
                ...
                def play_worker():
                    try:
                        with wave.open(audio_path, "rb") as wf:
                            ...
                            self.nao.speaker.request(
                                AudioRequest(sample_rate=sr, waveform=data)
                            )
                    except Exception as e:
            ...
            if "robot_command" in payload:
                ...
                if "motion" in cmd and cmd["motion"]:
                    ...
                    if "/" not in motion_val:
                        ...
                        if resolved:
                            self.logger.info(
                                f"Resolved motion intent '{motion_val}' to: {resolved}"
                            )
                            motion_val = resolved
                        else:
                            self.logger.warning(
                                f"Could not resolve motion intent: {motion_val}"
                            )
    ...
    def run(self):
        ...
        try:
            ...
            for i, flow_id in enumerate(self.flows):
                ...
                self.logger.info(
                    f"--- Starting Flow Cycle {i+1}/{len(self.flows)}: {flow_id} ---"
                )
                ...
                while flow_active and not self.shutdown_event.is_set():
                    ...
                    # Send to Dialogflow
                    resp = self.dialogflow_client.detect_intent_text(
                        user_text, self.session_id
                    )
    ...
    def process_turn_response(self, parsed):
        ...
        if not story_text:
            ...
            if (
                "Once upon a time" in all_text
                or "Generating Story" in all_text
                or "The huntsman" in all_text
            ):
